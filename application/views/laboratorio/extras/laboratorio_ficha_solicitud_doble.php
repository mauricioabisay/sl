...
<div id="laboratorio_solicitud">
	<div class="lineal">
		...
		<?php echo $laboratorio_sol->sintomas>0?'Sí':'No';?>
	</div>
	<div class="lineal" <?php (($laboratorio_sol->sintomas>0))?'':'style="display:none"';?>>
		<label>&iquest;Cu&aacute;ntos s&iacute;ntomas?</label>
		<?php echo $laboratorio_sol->sintomas;?>
	</div>
	<div class="lineal">
		<label>Fecha de Solicitud (Fecha en que se comunicó al paciente los estudios a realizar):</label>
		<?php $fecha_s = DateTime::createFromFormat('Y-m-d',$laboratorio_sol->fecha_solicitud);echo ($fecha_s)?$fecha_s->format('d-m-Y'):'';?>
	</div>
	<div>
		<label>Estudios a solicitados:</label>
		<?php $estudios_std = explode(',', $laboratorio_sol->especificacion);?>
		<?php foreach($laboratorios as $lab){?>
				<?php if(stristr($laboratorio_sol->especificacion,$lab->id)&&($lab->id!='otros')){?>
					<span class="lineal"><label><strong><?php echo $lab->nombre;?></strong></label></span>
				<?php }?>
		<?php }?>
		<?php $estudios_otro = explode(',', $laboratorio_sol->otros);$num=sizeof($estudios_otro);?>
