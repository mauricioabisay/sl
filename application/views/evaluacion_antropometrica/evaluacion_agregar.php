...
<form id="formulario" name="formulario" action="<?php echo site_url();?>/evaluacion/agregar/<?php echo $id_paciente;?>" method="post" enctype="multipart/form-data" style="width: 100%">
	...
	<fieldset>
		...
		<?php 
			if($edad<2){
				$this->load->view('evaluacion_antropometrica/extras/evaluacion_agregar_0a2');
			}else if($edad<5){
				$this->load->view('evaluacion_antropometrica/extras/evaluacion_agregar_2a5');
			}else if($edad<7){
				$this->load->view('evaluacion_antropometrica/extras/evaluacion_agregar_5a7');
			}else if($edad<12){
				$this->load->view('evaluacion_antropometrica/extras/evaluacion_agregar_7a12');
			}else{
				$this->load->view('evaluacion_antropometrica/extras/evaluacion_agregar_12a18');
			}
		?>
	</fieldset>
	<fieldset>
		...
		<div class="lineal_eval">
			...
			<input type="text" name="presion_sis" maxlength="3" size="3" value="<?php echo set_value('presion_sis');?>"/>
			<strong>/</strong>
			<input type="text" name="presion_dia" maxlength="3" size="3" value="<?php echo set_value('presion_dia');?>"/>
			<strong>mm Hg</strong>
		...
		<div class="lineal_eval">
			<label>Glucosa:</label>
			<input type="text" name="glucosa" maxlength="3" size="3" value="<?php echo set_value('glucosa');?>"/>
			<strong>mg/dl</strong>
	...
	<fieldset>
		...
		<div class="lineal_eval">
			...
			<input type="text" name="peso_saludable" maxlength="6" size="5" value="<?php echo set_value('peso_saludable');?>" />
			<strong>kgs.</strong>
			<?php echo form_error('peso_saludable');?>
		</div>
		<div class="lineal_eval">
			<label>Peso Meta:</label>
			<input type="text" name="peso_meta" maxlength="6" size="5" value="<?php echo set_value('peso_meta');?>" />
			<strong>kgs.</strong>
	...
	<?php if($edad >= 7){?>
	<div id="tanita" class="columnaizq" style="margin-left:5em; <?php echo (($this->input->post())&&($this->input->post('tanita'))=='Si')?'': 'display: none;';?>">
		<input type="hidden" id="tanita_boolean" name="tanita" value="<?php echo set_value('tanita','No');?>" />
		<?php $this->load->view('evaluacion_antropometrica/extras/evaluacion_tanita');?>
	</div>
	<?php }?>
</form>
<script>
